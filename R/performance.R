...
setMethod("performance", signature(x="FLQuants"),
  function(x, indicators, refpts=FLPar(), years=dims(x[[1]])$maxyear,
    ...
    # CHECK indicators as named list if otherwise

    if(is(indicators[[1]], "formula")) {
      warning("Passed a single indicator with no name, set to 'name'")
      indicators <- setNames(list(indicators), nm=indicators$name)
    }
...
# trace("performance", browser, exit=browser, signature = c("FLQuants"))
# untrace("performance", signature = c("FLQuants"))
