...
- `append(FLom, FLom, after)` method to join two operating models at a year boundary.
- - `vb(FLonmbf)` method computes for each biol using corresponding fisheries.
...
- The `setcontrol` function allows changes on the fly to an mpCtrl object, for example
  inside a call to `mp()`, e.g. `mp(om, control=setcontrol(ctrl, hcr=list(min=0.1))` to
  change the existing value for the `min` argument to a new one.
