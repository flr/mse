...
# TEST: RUN mps w/one hcr argument
...
# TEST: reports years in progress bar
test_that("mps runs with one hcr argument on multiple cores with performance", {
  ...
  tes14 <- mps(om=om, control=control, args=list(iy=2025, fy=2028),
    hcr=list(trigger=seq(12e3, 16e3, length=4)), perf=TRUE)

  expect_s3_class(tes14, "data.table")

})


# TEST: RUN mps w/two hcr arguments
