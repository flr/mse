...
dat=transform(ldply(oms[1:12],function(x) as.data.frame(window(ssb(x),start=75,end=100),drop=TRUE)),
...
dat=transform(ldply(oms[1:12],function(x) as.data.frame(window(fbar(x),start=75,end=100),drop=TRUE)),
    Scenario=factor(.id,levels=c("White Box","White Box Noise in M","White Box Red Noise in M",
                                 "Grey Box","Grey Box Noise in M","Grey Box Red Noise in M",
                                 "Black Box","Black Box Noise in M","Black Box Red Noise in M",
                                 "Perfect","Perfect Noise in M","Perfect Red Noise in M")))
...
dat=transform(ldply(oms[1:12],function(x) as.data.frame(window(catch(x),start=75,end=100),drop=TRUE)),
    Scenario=factor(.id,levels=c("White Box","White Box Noise in M","White Box Red Noise in M",
                                 "Grey Box","Grey Box Noise in M","Grey Box Red Noise in M",
                                 "Black Box","Black Box Noise in M","Black Box Red Noise in M",
                                 "Perfect","Perfect Noise in M","Perfect Red Noise in M")))
...
```{r}
plot(FLStocks(llply(oms[sort(names(oms))[seq(1,15,3)]],function(x) x[,ac(75:100)])))
```

```{r}
plot(FLStocks(llply(oms[sort(names(oms))[seq(2,15,3)]],function(x) x[,ac(75:100)])))
```

```{r}

plot(FLStocks(llply(oms[sort(names(oms))[seq(3,15,3)]],function(x) x[,ac(75:100)])))
```
...
key=data.frame(
  ...
  mp=rep(c("Perfect","White Box","Grey Box","Black Box","Ref"),each=3),
  om=rep(c("None","Noise","Red Noise"),5),stringsAsFactors=FALSE)

ss=transform(merge(ss,key,by=".id"),om=factor(om,levels=c("None","Noise","Red Noise")))
ggplot(subset(ss,qname=="ssb"&mp%in%c("White Box","Grey Box","Black Box")))+
  geom_boxplot(aes(factor(year),data))+
  facet_grid(mp~om)

dat=ddply(subset(ss,qname=="ssb"&mp%in%c("White Box","Grey Box","Black Box")),
          .(mp,om,year), with, data.frame(p=mean(ifelse(data>=1,1,0))))

ggplot(dat)+
  geom_line(aes(year,p))+
  facet_grid(mp~om)

dat=ddply(subset(ss,qname=="yld"&mp%in%c("White Box","Grey Box","Black Box")&year<=90),
